defmodule HTTPDigexTest do
  ...
  test "create_digest/5 creates auth digest correctly" do
    ...
    assert Regex.match?(
             ~r/^Digest username="test", realm="test realm", nonce="[0-9a-f]{32}", uri="\/someurl", response="[0-9a-f]{32}", opaque=""$/,
             auth_digest
           )
    ...
    assert Regex.match?(
             ~r/^Digest username="test", realm="test realm", nonce="[0-9a-f]{32}", uri="\/", response="[0-9a-f]{32}", opaque=""$/,
             auth_digest
           )
